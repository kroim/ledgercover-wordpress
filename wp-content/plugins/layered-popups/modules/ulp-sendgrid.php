...
class ulp_sendgrid_class {
					...
					<tr>
						...
						<td style="vertical-align: middle;">
							...
							<table style="min-width: 280px; width: 50%;">
								<tr>
									<td style="width: 200px;"><strong>'.__('Email', 'ulp').':</strong></td>
									<td>
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['sendgrid_enable'] == 'on') {
			$data = array(
				'list_ids' => array($popup_options['sendgrid_list_id']),
				'contacts' => array(
					array(
			...
			foreach ($popup_options['sendgrid_fields'] as $key => $value) {
				if (!empty($value)) {
					$data['contacts'][0][$key] = strtr($value, $_subscriber);
				}
			}
			$result = $this->connect($popup_options['sendgrid_api_key'], 'contacts', $data, 'PUT');
		}
	}
	function show_lists() {
		global $wpdb;
		...
		if (is_array($result)) {
			...
			<table style="min-width: 280px; width: 50%;">';
					foreach ($result['reserved_fields'] as $field) {
						if (is_array($field)) {
							if (array_key_exists('name', $field) && !array_key_exists('read_only', $field) && !in_array($field['name'], array('email', 'first_name', 'last_name'))) {
								$fields .= '
				<tr>
					<td style="width: 200px;"><strong>'.esc_html($field['name']).':</strong></td>
					<td>
